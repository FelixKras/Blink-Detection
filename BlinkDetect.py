# -*- coding: utf-8 -*-
"""
Created on Sat Oct 27 21:00:55 2018
...
def ImproveLowLight_Average(frameIn):

	NumFramesToAverage = 10
	average_frame = np.zeros(frameIn.shape)
...
def ImproveLowLight_CLAHE(frame):
...
while True:
	...
	if improve_low_light==1:
		gray=ImproveLowLight_Average(frame)
	elif improve_low_light==2:
		gray=ImproveLowLight_CLAHE(frame)
	elif improve_low_light==3:
		gray = ImproveLowLight_Average(frame)
		gray = ImproveLowLight_CLAHE(gray)
	else:
		gray= cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
